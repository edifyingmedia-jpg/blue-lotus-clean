...
export default function TWIN() {
  ...
  const handleSend = () => {
    ...
    // Intelligence: Check for build intent to trigger the Success Architect Proposal
    const lowInput = input.toLowerCase();
    if (lowInput.includes("build") || lowInput.includes("app") || lowInput.includes("scaffold")) {
      
      // Pass isPrimeActive so the proposal returns the private Prime Directives
      ...
      setTimeout(() => {
        setMessages(prev => [
          ...
          { 
            role: 'twin', 
            content: isPrimeActive 
              ? "PRIME_STRATEGY_GENERATED: Review the directives below." 
              : "Proposal generated. My analysis shows a high probability of success.",
            proposal: proposal 
          }
        ]);
      }, 600);
          ...
          <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            <div className={`max-w-[85%] p-3 rounded-lg text-[11px] leading-relaxed ${
              msg.role === 'user' 
                ...
                : 'bg-cyan-950/20 text-cyan-50 border border-cyan-900/30'
            }`}>
              {msg.content}
            </div>
            ...
            {/* Proposal Rendering */}
            {msg.proposal && (
              <div className={`mt-2 w-full p-3 bg-black border rounded-md space-y-2 ${
                isPrimeActive ? 'border-purple-500/30' : 'border-white/10'
              }`}>
                <h4 className={`text-[10px] font-bold uppercase ${
                  isPrimeActive ? 'text-purple-400' : 'text-cyan-400'
                }`}>
                  {msg.proposal.title}
                </h4>
                <p className="text-[9px] text-white/50 italic">{msg.proposal.description}</p>
                
                {/* Prime-Only Strategic View */}
